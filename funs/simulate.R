simulate_strategies <- function(l_params, # list of parameters to be varied in sensitivity analysis
                                ...
                                costs_trt = 128, # treatment costs per patient per mechanically ventilated ICU day
                                ...
                                explore_intervention = FALSE, # set to true if you want scenario analyses on intervention effects
                                ceac = FALSE, # set to true if output should be suitable for a cost effectiveness acceptability curve
                                ...
                                sd_time_to_mort = 4.44, #standard deviation of mortality in recovery state
                                ...
                                costs_gen_ward_fixed = 442.78, # cost per day in the general ward
                                discount_rate = 0.03) {  
  with(as.list(l_params), {
    ...
    #Draw from a gamma distribution to obtain time to death in recovery state (separately for non/mildly disabled and disabled)
    ...
    # Age intervals
    age_brackets <- c(18, 25, 35, 45, 55, 65, 75, Inf)
    utilities_f <- c(utilities_post_recovery18_f, utilities_post_recovery25_f, 
                     utilities_post_recovery35_f, utilities_post_recovery45_f, 
                     utilities_post_recovery55_f, utilities_post_recovery65_f,
                     utilities_post_recovery75_f)
    utilities_m <- c(utilities_post_recovery18_m, utilities_post_recovery25_m, 
                     utilities_post_recovery35_m, utilities_post_recovery45_m,
                     utilities_post_recovery55_m, utilities_post_recovery65_m,
                     utilities_post_recovery75_m)
    
    # Get index of the appropriate utility vector based on mean_age
    index_f <- findInterval(mean_age, age_brackets)
    index_m <- findInterval(mean_age, age_brackets)
    
    # Calculate utility for hospital state
    utility_hos_f <- utilities_f[index_f]
    utility_hos_m <- utilities_m[index_m]
